...
export const SETTING_DEFS: readonly SettingDef[] = [
  ...
  // ---- pictures --------------------------------------------------------
  //
  // Added because the renderer was drawing a gradient. Every script has always
  // produced a scene_prompt describing the image the episode wants, and until
  // now nothing consumed it — the pipeline described the picture and then drew
  // a coloured wash instead.
  {
    key: "images_enabled",
    group: "writing",
    label: "Generate the pictures",
    kind: "boolean",
    secret: false,
    fallbackEnv: "IMAGES_ENABLED",
    fallback: "true",
    help: "On means each episode is drawn from its own narration, one image per shot. Off falls back to whatever is filed under assets/backgrounds, then to a plain gradient.",
  },
  {
    key: "image_provider",
    group: "writing",
    label: "Where the pictures come from",
    kind: "select",
    secret: false,
    options: ["pollinations", "cloudflare", "gemini"],
    fallbackEnv: "IMAGE_PROVIDER",
    fallback: "pollinations",
    help: "pollinations needs no account and works immediately, but returns 576x1024 from a small model. cloudflare is free — 10,000 neurons a day — on a better model, and needs an account ID and token below. gemini is the best of the three and the only one that is NOT free: its image models refuse the free tier outright with limit: 0, so it needs billing enabled.",
  },
  {
    ...
    fallback: "",
    help: "Only needed when the image source is cloudflare. Found on the right of any domain's overview page, or in the Workers & Pages URL.",
  },
  {
    ...
    kind: "password",
    secret: true,
    fallbackEnv: "CLOUDFLARE_API_TOKEN",
    fallback: "",
    help: "Create one at dash.cloudflare.com under My Profile then API Tokens, with the Workers AI read permission. No card required.",
  },
  {
    key: "cloudflare_image_model",
    group: "connections",
    label: "Cloudflare image model",
    kind: "text",
    secret: false,
    fallbackEnv: "CLOUDFLARE_IMAGE_MODEL",
    fallback: "@cf/black-forest-labs/flux-1-schnell",
    help: "Takes a prompt and a seed and nothing else, so the image comes back square and is cropped to 9:16 by the Ken Burns move.",
  },
  {
    key: "image_model",
    group: "connections",
  ...
  {
    key: "image_model_fallbacks",
    group: "connections",
    label: "Image model fallbacks",
    kind: "text",
    secret: false,
    fallbackEnv: "IMAGE_MODEL_FALLBACKS",
    fallback: "gemini-2.5-flash-image",
    help: "Tried in order when the one above is out of quota or the name is wrong. Comma separated.",
  },
  {
    key: "image_seconds_per_shot",
    group: "writing",
    ...
    min: 1.5,
    max: 15,
    fallbackEnv: "IMAGE_SECONDS_PER_SHOT",
    fallback: "3",
    help: "How long each image holds. A measured reference in this genre changes picture about every two seconds; three keeps that feel while asking for fewer images.",
  },
  {
    key: "image_max_shots",
    group: "writing",
    label: "Most pictures per video",
    kind: "number",
    secret: false,
    min: 1,
    max: 40,
    fallbackEnv: "IMAGE_MAX_SHOTS",
    fallback: "16",
    help: "A ceiling on the images one video may ask for, so a long narration cannot spend the day's quota in one render.",
  },
  {
    key: "image_pace_ms",
    group: "writing",
    label: "Wait between pictures (ms)",
    kind: "number",
    secret: false,
    min: 0,
    max: 120000,
    fallbackEnv: "IMAGE_PACE_MS",
    fallback: "1500",
    help: "How long to wait between pictures. Right for pollinations and cloudflare, which are not metered per minute. Gemini is limited to about two a minute, so raise this to 31000 if you switch to it.",
  },
  {
    key: "image_crossfade_seconds",
    group: "writing",
    label: "Dissolve length (seconds)",
    kind: "number",
    secret: false,
    min: 0,
    max: 2,
    fallbackEnv: "IMAGE_CROSSFADE_SECONDS",
    fallback: "0.6",
    help: "How long one picture takes to become the next. The reference video has no hard cuts anywhere — every change is a dissolve.",
  },
  {
    key: "auto_publish",
    group: "publishing",
